<script>
  ...
  async function loadData(){
    ...
    loading = true;
    let data = await fetch(`https://events.startupmission.in/api/event/wss-2023/agenda/expanded`).then(response => response.json());
...
<div id="event-sched1" data-title="Event Schedule" class="">
  {#each Object.entries(agenda_list) as [date, agendas]}
    ...
    {#if date == active_tab}
      ...
      <div class="text-center mb-10">Venue: Theater - Digital Hub, Kerala Technology Innovation Zone, Kochi		</div>

      <div class="w-full max-w-4xl mx-auto mb-5">

        {#each Object.entries(agendas) as [time, agenda] }
          
          <div class="bg-whie alternate-row px-5 py-3">
